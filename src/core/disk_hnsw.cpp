...
// ============================================================
// OPT-002: PQ top-1 — 用于 query 重排序
// ============================================================
uint32_t DiskHNSW::getPQTop1(const float* query) {
    if (pq_codes_.empty()) return 0;
    ...
    // 复用 buildPqDistTable 的 thread_local table (同一个 static thread_local)
    static thread_local std::vector<float> tl_scan_table;
    auto& tbl = tl_scan_table;
    // buildPqDistTable 用的是自己的 static thread_local, 我们无法直接访问
    // 解决: 手动重建距离表 (代码从 buildPqDistTable 复制)
    tbl.resize((size_t)M * ksub);
    const float* cb = pq_codebook_.data();
    const uint32_t dsub = pq_params_.dsub;
    ...
    if (dsub == 4) {
        for (uint32_t m = 0; m < M; m++) {
            const float* q_sub = query + (size_t)m * 4;
            const float* cb_m = cb + (size_t)m * ksub * 4;
            float* t = &tbl[(size_t)m * ksub];
            __m128 qv = _mm_loadu_ps(q_sub);
            __m256 q2 = _mm256_insertf128_ps(_mm256_castps128_ps256(qv), qv, 1);
            for (uint32_t c = 0; c < ksub; c += 2) {
                __m256 cv0 = _mm256_loadu_ps(cb_m + (size_t)c * 4);
                __m256 cv1 = _mm256_loadu_ps(cb_m + ((size_t)c + 1) * 4);
                __m256 diff0 = _mm256_sub_ps(q2, cv0);
                __m256 diff1 = _mm256_sub_ps(q2, cv1);
                __m256 sq0 = _mm256_mul_ps(diff0, diff0);
                __m256 sq1 = _mm256_mul_ps(diff1, diff1);
                __m128 s0lo = _mm256_castps256_ps128(sq0);
                __m128 s0hi = _mm256_extractf128_ps(sq0, 1);
                __m128 s1lo = _mm256_castps256_ps128(sq1);
                __m128 s1hi = _mm256_extractf128_ps(sq1, 1);
                t[c]   = _mm_cvtss_f32(_mm_add_ps(_mm_hadd_ps(s0lo, s0hi), _mm_setzero_ps()));
                t[c+1] = _mm_cvtss_f32(_mm_add_ps(_mm_hadd_ps(s1lo, s1hi), _mm_setzero_ps()));
            }
        }
    } else {
        for (uint32_t m = 0; m < M; m++) {
            ...
            for (uint32_t c = 0; c < ksub; c++) {
                float dist = 0;
                const float* cent = cb_m + (size_t)c * dsub;
                for (uint32_t j = 0; j < dsub; j++) {
                    float d = q_sub[j] - cent[j];
                    dist += d * d;
                }
                t[c] = dist;
            }
    ...
    // AVX2 扫描所有节点找 top-1
    const float* t = tbl.data();
    float best_dist = 1e30f;
    uint32_t best_id = 0;

    for (uint32_t nid = 0; nid < N; nid++) {
        const uint8_t* code = &pq_codes_[(size_t)nid * M];
        __m256 acc = _mm256_setzero_ps();
        uint32_t m = 0;
        for (; m + 8 <= M; m += 8) {
            __m256i idx = _mm256_set_epi32(
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            if (kFinePread) {
                ...
                // OPT-001: 批量 pread 合并 — 排序 pages_needed, 合并连续页为单次大 pread
                auto tp0 = std::chrono::high_resolution_clock::now();

                // 排序 page ids
                std::vector<uint32_t> sorted_pages(pages_needed.begin(), pages_needed.end());
                std::sort(sorted_pages.begin(), sorted_pages.end());

                // 合并连续页为区间 [start, end]
                struct PageRange { uint32_t start; uint32_t end; }; // [start, end] inclusive
                std::vector<PageRange> ranges;
                for (uint32_t pg : sorted_pages) {
                    if (!ranges.empty() && ranges.back().end + 1 == pg) {
                        ranges.back().end = pg; // 扩展连续区间
                    } else {
                        ranges.push_back({pg, pg});
                    }
                }

                // 每个区间一次性 pread
                std::unordered_map<uint32_t, std::unique_ptr<char[]>> page_cache;
                page_cache.reserve(sorted_pages.size());
                for (const auto& rng : ranges) {
                    uint32_t num_pages = rng.end - rng.start + 1;
                    size_t read_bytes = (size_t)num_pages * 4096;
                    auto buf = std::make_unique<char[]>(read_bytes);
                    ssize_t r = pread(vec_blocks_fd_, buf.get(), read_bytes, (off_t)rng.start << 12);
                    if (r > 0) {
                        // 拆分回单页 map, 供后续查询
                        for (uint32_t pg = rng.start; pg <= rng.end; pg++) {
                            uint32_t idx = pg - rng.start;
                            page_cache[pg] = std::make_unique<char[]>(4096);
                            std::memcpy(page_cache[pg].get(), buf.get() + (size_t)idx * 4096, 4096);
                        }
                    }
                }
