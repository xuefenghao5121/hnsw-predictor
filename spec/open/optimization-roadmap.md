# Optimization Roadmap — 实验结果与待验证项

> 更新: 2026-07-30 (DEC-046~048 实验完成)
> 状态: 16 个 idea 已验证, 14 个待实现

---

## 已验证 Idea 总览

### ✅ 采纳 (有正收益)

| ID | 优化 | 收益 | 决策 |
|----|------|------|------|
| DEC-034 | VisitedList uint32→uint8 | 2x QPS (10M) | ✅ |
| DEC-035 | FINE_PREAD 多线程路径 | recall 70%→95% | ✅ |
| DEC-036 | PQ dsub=3 SIMD | +5% QPS | ✅ |
| DEC-037 | cgroup 目标 1GB→2GB | 解除内存瓶颈 | ✅ |
| DEC-047 | P1 批量 pread 合并 | +9%~146% QPS | ✅ |

### ❌ 否决 (无收益或负收益)

| ID | 优化 | 结果 | 根因 |
|----|------|------|------|
| DEC-019 | Dynamic Width | PQ 搜索不收敛 | 架构特性 |
| DEC-038 | 图裁剪 10M | 0% recall loss, 0% QPS gain | CSR 非瓶颈 |
| DEC-046 | CSR cache 增大 256→512MB | hit_rate 68.4%→68.4% | 工作集已在 cache 内 |
| DEC-048 | P2 query 重排序 | QPS 反降 | 预计算开销 > 缓存收益 |

### ✅ 完成但收益边际 (保留, 待更大规模验证)

| ID | 优化 | 1M 收益 | 待验证 |
|----|------|---------|--------|
| DEC-018 | Page Shuffle | +2.1% | 10M 可能 +25% |
| DEC-017 | Page Search | +2% (冷态) | 10M 待定 |

---

## 待实现 Idea (按优先级)

### 🔴 优先级 1: 全量放开性能优化 (SLA-006)

> 目标: DiskHNSW 1T QPS 2492 → 8000+ (hnswlib 70%)
> 当前差距: 4.6x, 根因: 两阶段搜索 (PhaseA 57% + Fine 38%)

| # | 优化 | 预期 | 难度 | 状态 |
|---|------|------|------|------|
| 1 | **PhaseA+Fine 融合** (DEC-045 P1) | -40% 延迟 | 🔴 高 | 未开始 |
| 2 | **PQ SIMD dsub=4** (DEC-036 ext) | -15% PhaseA | 🟡 中 | 未开始 |
| 3 | **flat_vec_cache 增大** (DEC-045 P4) | -10% Fine | 🟢 低 | 未开始 |

### 🟡 优先级 2: 紧凑部署优化 (1.5GB cgroup)

| # | 优化 | 预期 | 难度 | 状态 |
|---|------|------|------|------|
| 4 | **io_uring 批量 Fine Rerank** (Q-006) | 1.5x @1.5GB | 🔴 高 | 未开始 |
| 5 | **1-hop CSR 预取** (P3-4) | 掩盖磁盘延迟 | 🟡 中 | 未开始 |
| 6 | **Page Shuffle @10M** (DEC-018) | I/O -25%? | 🟡 中 | 未开始 |

### 🟢 优先级 3: 内存压缩 (P3 100M 前置)

| # | 优化 | 效果 | 难度 | 状态 |
|---|------|------|------|------|
| 7 | **PQ codes mmap** (P3-5) | 减少 304MB 匿名 | 🟡 中 | 未开始 |
| 8 | **上层向量 PQ 编码** (P3-6) | 减少 228MB 匿名 (10M) | 🟡 中 | 未开始 |
| 9 | **路由表内存优化** (P3-8) | 减少 ~40MB | 🟢 低 | 未开始 |

### ⚪ 优先级 4: 长期探索 (P4-P5)

| # | 优化 | 效果 | 难度 | 状态 |
|---|------|------|------|------|
| 10 | **100M 数据集** (P3-7) | 验证规模上限 | 🔴 高 | 未开始 |
| 11 | **SPDK 用户态 I/O** (DEC-027) | 2-3x I/O 带宽 | 🔴 高 | P4 探索 |
| 12 | **学习式剪枝 GBDT** (DEC-028) | 1.1-1.6x 吞吐 | 🔴 高 | P2.5 探索 |
| 13 | **GPU/PMEM/NUMA** (P5) | 未知 | 🔴 高 | P5 探索 |
| 14 | **Q-002 学习式剪枝可行性** | - | 🔴 高 | 开放 |

---

## 已关闭的 Open Question

| ID | 问题 | 结论 |
|----|------|------|
| Q-001 | 图遍历规模上限 | 10M 可行 ✅, 100M 需关注 CSR 4.7GB 上磁盘 |
| Q-003 | 数据集规模选择 | SIFT1M + DEEP10M ✅ |
| Q-004 | PQ M 值选择 | M=32 ✅ |
| Q-005 | 路由表方向 | dual-route-table ✅ |
| Q-006 | 批处理优先级 | P1 pread 合并已验证 ✅, io_uring 待做 |
| Q-007 | SLA 重新定义 | SLA-006 双维度 ✅ |
