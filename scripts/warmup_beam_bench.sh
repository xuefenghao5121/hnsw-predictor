#!/bin/bash
# warmup_beam_bench.sh - warmup 对齐后的 beam search benchmark
# 先空跑预热顶高频，再背靠背跑 F0 + F2-blocking + F2-beam-8
set -e
...
echo "=========================================="
echo "  Warmup 对齐 Beam Search Benchmark"
...
# Step 2: 背靠背跑正式 benchmark (同一高频窗口)
echo ""
echo "[Benchmark] Running 200 queries..."
./build/benchmark_overlap $GRAPH $BFS $BLOCKS $ROUTE $DATA $QUERY $GT $K $EF $NQ 2>&1 | tee logs/beam_warmup_bench.log

echo ""
echo "Done. Results saved to logs/beam_warmup_bench.log"
